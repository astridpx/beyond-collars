...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['pet_id'])) {
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['found_name']) && isset($_POST['found_details']) && isset($_FILES['found_photo']) && isset($_FILES['valid_id_photo']) && isset($_POST['termsCheckbox'])) {
        ...
        $insert_sql = "INSERT INTO found_pets (pet_id, found_name, found_details, found_photo, valid_id_photo, status) VALUES (?, ?, ?, ?, ?, ?)";
        $stmt = $conn->prepare($insert_sql);
        // Set the status as 0 (not resolved) when inserting data
        $status = 0;
        $stmt->bind_param("issssi", $found_pet_id, $found_name, $found_details, $found_photo_data, $valid_id_photo_data, $status);
